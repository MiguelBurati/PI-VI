...
export const UserProvider = ({ children }) => {
    ...
    const [users, setUsers] = useState(dbUsuarios);
    ...
    const loadUsersAndAuth = useCallback(async () => {
        ...
        try {
            ...
            if (!parsedUsers['user_1234']) {
                parsedUsers = { ...parsedUsers, ...dbUsuarios };
            }
    ...
    const resetUsers = async () => {
        setIsLoading(true);
        try {
            await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(dbUsuarios));
            setUsers(dbUsuarios);
            
            if (dbUsuarios['user_1234']) {
                setCurrentUser(dbUsuarios['user_1234']);
